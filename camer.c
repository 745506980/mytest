...
int read_frame(Encode *en, sps_pps * sp, uint32_t pixformat, uint32_t timer)
{
	struct v4l2_buffer buf;
	int ret = 0;
	CLEAN(buf);
...
#endif 
#if 1
	ret = Encode_frame(en, pixformat, sp, buffer[0].start, WIDTH, HEIGHT, timer);	
	if (ret == -1){
...
int process_frame(Encode * en, sps_pps * sp, uint32_t pixformat, uint32_t timer)
{
...
//	gettimeofday(&start, NULL);
	...
	if(read_frame(en, sp, pixformat, timer) == -1)
	{
