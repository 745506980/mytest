...
/*	if (pixformat == V4L2_PIX_FMT_YUYV)
	...
	/*图像质量控制*/
//	en->param.rc.f_rf_constant = 25; //实际质量，越大图像越花，越小越清晰
//	en->param.rc.f_rf_constant_max = 45; //图像质量的最大值
...
//	en->param.i_log_level = X264_LOG_DEBUG;	//打印编码信息
	...
	if (!ConstantBitRate)
	{
		...
		en->param.rc.i_vbv_max_bitrate = (int)bitrate; //平均码率下，最大瞬时码率，默认0
		en->param.rc.i_vbv_buffer_size = (int)bitrate;
	}
	else{
	...
	//设置编码复杂度
//	x264_param_apply_profile(&en->param, "hight");
...
int 
Encode_frame(Encode *en, uint32_t pixformat, sps_pps * sp, uint8_t *frame, uint32_t width, uint32_t height, uint32_t timer)
{
...
/* 获取编码后的数据*/		
	...
	//			printf("--------------------\n");	
				/*发送编码过后的数据*/
#if 1				
				int ret = Send_h264_packet(en->nal[i].p_payload + 4, sp, en->nal[i].i_payload - 4, type, timer);
				if (ret < 0){
